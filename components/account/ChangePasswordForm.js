...
export default function ChangePasswordForm({
    ...
    const onSubmit = async () => {
      if (!validateForm()) {
        ...
        if (!resultUpdatePassword.statusResponse) {
          setLoading(false);
          setErrorNewPassword("Hubo un problema cambiando la contraseña, por favor intente más tarede.");
          return;
        }
    ...
    const validateForm = () => {
      ...
      if (isEmpty(currentPassword)) {
        setErrorCurrentPassword("Debes de ingresar tu contraseña actual.");
        isValid = false;
      }
    //   if (currentPassword != password)  {
    //     setErrorCurrentPassword("Las contraseña introducida, no coincide con la actual.");
    //   isValid = false;
    // }
    ...
    if (size(confirmPassword) <6) {
        setErrorConfirmPassword("Debes de ingresar una Confirmación de tu contraseña de al menos 6 carácteres.");
      isValid = false;
    }

    if (newPassword != confirmPassword)  {
        setErrorConfirmPassword("Las Nueva contraseña y la confiramación no son iguales.");
        setErrorNewPassword("Las Nueva contraseña y la confiramación no son iguales.");
      isValid = false;
    }
